            ...
            for exec in executables {
                match exec {
                    Ok(exec) => {
                        if !exec.path().is_dir() {
                            let mut filename =
                                exec.file_name().to_str().unwrap().to_owned().to_lowercase();
                            ...
                            {
                                if !is_linux_binary {
                                    filename = filename.rsplit_once(".").unwrap().0.to_owned();
                                }
...
fn main() {
    ...
    loop {
        ...
        // Magic!
        let mut input_stream = clean_args
            .iter()
            .map(|s| &**s)
            .collect::<Vec<&str>>()
            .into_iter();
        ...
        match command.command {
            CliCommand::Type => {
                let c: CliCommandComp<'_> =
                    CliCommandComp::builtin_map_command(input_stream.next());
                match c.command {
                    CliCommand::Invalid => {
            ...
            CliCommand::Invalid => {
                ...
                if paths.contains_key(&cmd) {
                    ...
                    let process = Command::new(command.orig).args(args).spawn();
                    match process {
                        Ok(mut process) => {
